    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .and()
                    ...
                    // authorizeRequests가 선언되어야만 antMatchers 오션은 사용할 수 있다.
                    .authorizeRequests()
                    // 권한 관리 대상을 지정하는 옵션
                ...
                // 여기서 authenticated()을 추가하여 나머지 URL들은 모두 인증된 사용자들에게만 허용한다.
                // 인증된 사용자 즉, 로그인한 사용자들을 이야기한다.
                .anyRequest().authenticated()
                .and()
                    // logout().logoutSuccessUrl("/") - 로그아웃 기능에 대한 여러설정의 진입점 / 로그아웃 성공시 "/"주소로 이동한다.
                    .logout()
                        .logoutSuccessUrl("/")
                .and()
                    ...
                    .oauth2Login()
                        ...
                        .userInfoEndpoint()
                            // userService - 소셜 로그인 성공 시 후속 조치를 진행할 UserService인터페이스의 구현체를 등록
                            // 리소스 서버(즉, 소셜서비스들)에서 사용자 정보를 가져온 상태에서 추가로 진행하고자 하는 기능을 명시할 수 있다.
